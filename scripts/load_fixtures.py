...
async def save_pizzas(pizzas: List[Dict[str, str]]) -> None:
    ...
    for pizza in pizzas:
        ...
        await created_pizza.ingredients.add(*list(filter(lambda ing: ing.name in pizza["ingredients"], ingredients)))
...
async def save_ingredients(pizzas: List[Dict[str, str]]) -> None:
    ...
    await PizzaIngredient.bulk_create([PizzaIngredient(name=name) for name in set(all_ingredients)])
...
async def load_fixtures() -> None:
    ...
    with open(Path(fixtures_dir, "pizzas.json"))as pizzas, open(Path(fixtures_dir, "pizza_extras.json")) as pizza_extras:
